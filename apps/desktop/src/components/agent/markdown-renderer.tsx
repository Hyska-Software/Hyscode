...
function extractTextContent(node: React.ReactNode): string {
  ...
  if (typeof node === 'object' && node !== null && 'props' in (node as object)) {
    return extractTextContent(
      ((node as React.ReactElement).props as { children?: React.ReactNode }).children,
    );
  }
  return '';
}
...
export function CodeBlock({
  ...
  if (
    ...
    children.trimStart().startsWith('Edit applied successfully.')
  ) {
    statusBadge = 'Edit applied successfully.';
    renderedChildren = children
      .trimStart()
      .slice('Edit applied successfully.'.length)
      .replace(/^\n/, '');
  }
  ...
  return (
    <div className="group/code relative my-2.5 overflow-hidden rounded-lg bg-[var(--color-surface)]">
      ...
      <div className="flex h-7 items-center justify-between border-b border-foreground/[0.06] bg-[var(--color-surface-raised)]/60 px-3">
        <div className="flex min-w-0 items-center gap-1.5">
          {filePath ? (
            ...
            <>
              ...
              <span className="ml-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">
                {lang || 'code'}
              </span>
            </>
          )}
        ...
        <button
          onClick={handleCopy}
          className="flex items-center gap-1 rounded-md px-2 py-1 text-[10px] text-muted-foreground/70 transition-all hover:bg-foreground/[0.04] hover:text-foreground"
        >
          {copied ? (
            <>
      ...
      <pre className="cursor-text select-text overflow-x-auto p-3.5 text-[11.5px] leading-[1.7]">
...
export const MARKDOWN_COMPONENTS = {
  ...
  p: ({ children }: { children?: React.ReactNode }) => (
    <p className="my-1.5 leading-[1.75]">{children}</p>
  ),
  ...
  h3: ({ children }: { children?: React.ReactNode }) => (
    <h3 className="mb-1 mt-3 text-[13px] font-semibold text-foreground">{children}</h3>
  ),
  h4: ({ children }: { children?: React.ReactNode }) => (
    <h4 className="mb-0.5 mt-2.5 text-[12.5px] font-semibold text-foreground/90">{children}</h4>
  ),
  h5: ({ children }: { children?: React.ReactNode }) => (
    <h5 className="mb-0.5 mt-2 text-[12px] font-medium text-foreground/85">{children}</h5>
  ),
  h6: ({ children }: { children?: React.ReactNode }) => (
    <h6 className="mb-0.5 mt-1.5 text-[11.5px] font-medium text-foreground/75">{children}</h6>
  ),

  // Lists
  ul: ({ children }: { children?: React.ReactNode }) => (
    <ul className="my-2 ml-4 list-disc space-y-1 marker:text-muted-foreground/40">{children}</ul>
  ),
  ol: ({ children }: { children?: React.ReactNode }) => (
    <ol className="my-2 ml-4 list-decimal space-y-1 marker:text-muted-foreground/40">{children}</ol>
  ),
  li: ({ children, className }: { children?: React.ReactNode; className?: string }) => {
    ...
    return (
      <li
        className={cn(
          'pl-0.5 text-foreground/85',
          isTaskItem && '-ml-4 flex list-none items-center gap-1.5',
        )}
      >
        {children}
      </li>
...
export const MarkdownContent = memo(function MarkdownContent({
  ...
  return (
    <div
      className={cn(
        'agent-markdown cursor-text select-text text-[13px] leading-[1.75] text-foreground/90',
        className,
      )}
    >
      <ReactMarkdown
